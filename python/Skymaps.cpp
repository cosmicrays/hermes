...
void declare_default_skymap_methods(py::class_<SKYMAP> c) {
	...
	c.def("setIntegrator",
	      [](SKYMAP &s, std::shared_ptr<IntegratorClass> i) {
		      s.setIntegrator(i);
	      });
...
void init_skymaps(py::module &m) {
	...
	declare_default_skymap_methods<DispersionMeasureSkymap,
				       QDispersionMeasure, QNumber>(dmskymap);

	// RotationMeasureSkymap
	py::class_<RotationMeasureSkymap,
		   std::shared_ptr<RotationMeasureSkymap>>
	...
	py::class_<InvertMask, std::shared_ptr<InvertMask>, SkymapMask>(
	    ...
	    .def(py::init<const std::shared_ptr<SkymapMask>>(),
		 py::arg("mask"));
	py::class_<MaskList, std::shared_ptr<MaskList>, SkymapMask>(m,
								    "MaskList")
	    .def(py::init<>())
	...
	py::class_<CircularWindow, std::shared_ptr<CircularWindow>, SkymapMask>(
	    m, "CircularWindow")
	    .def(py::init<const QDirection &, const QAngle &>(),
		 py::arg("centre"), py::arg("aperature"));
}
