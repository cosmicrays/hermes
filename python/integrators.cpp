...
void init_integrators(py::module &m) {
	...
	NEW_INTEGRATOR(dmintegrator, "DispersionMeasureIntegrator",
		       DispersionMeasureIntegrator, QDispersionMeasure,
		       QNumber);
	dmintegrator.def(
	...
	declare_default_integrator_methods<RotationMeasureIntegrator>(
	    rmintegrator);
	...
	declare_default_integrator_methods<SynchroIntegrator>(
	    synchrointegrator);
	...
	NEW_INTEGRATOR(icintegrator, "InverseComptonIntegrator",
		       InverseComptonIntegrator, QDifferentialIntensity,
		       QEnergy);
	icintegrator.def(
	    py::init<const std::shared_ptr<cosmicrays::CosmicRayDensity>,
		     const std::shared_ptr<photonfields::PhotonField>,
		     const std::shared_ptr<
			 interactions::DifferentialCrossSection>>());
	declare_default_integrator_methods<InverseComptonIntegrator>(
	    icintegrator);
	...
	pizerointegrator.def(
	    py::init<const std::vector<
			 std::shared_ptr<cosmicrays::CosmicRayDensity>>,
		     const std::shared_ptr<neutralgas::RingModel>,
		     const std::shared_ptr<
			 interactions::DifferentialCrossSection>>());
