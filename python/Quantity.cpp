...
template <typename Q>
void declare_quantity(py::module &m, std::string const &typestr,
		      std::string const &suffix, AstroPy &apyu) {
	py::class_<Q>(m, typestr.c_str())
	    .def(py::init<const double>())
	    .def(
		"__lt__",
		[](const Q &self, const Q &other) -> bool {
			return self < other;
		},
		py::is_operator())
	    .def(
		...
		py::is_operator())
	    .def(
		"__neg__", [](const Q &self) -> Q { return -1 * self; },
		py::is_operator())
	    .def(
		"__add__",
		[](const Q &self, const Q &other) -> Q { return self + other; },
		py::is_operator())
	    .def(
		"__sub__",
		[](const Q &self, const Q &other) -> Q { return self - other; },
	    ...
	    .def(
		"__truediv__",
		[](const Q &self, const Q &other) -> double {
			return static_cast<double>(self / other);
		},
		py::is_operator())
	    .def(
		"__mul__",
		[](const Q &self, const int a) -> Q { return a * self; },
		py::is_operator())
	    .def(
		"__mul__",
		[](const Q &self, const double a) -> Q { return a * self; },
		py::is_operator())
	    .def(
		"__rmul__",
		[](const Q &self, const int a) -> Q { return a * self; },
	    ...
	    .def(
		"__truediv__",
		[](const Q &self, const int a) -> Q { return self / a; },
		py::is_operator())
	    .def(
		"__truediv__",
		[](const Q &self, const double a) -> Q { return self / a; },
		py::is_operator())
	    .def(
		"__abs__", [](const Q &self) -> Q { return fabs(self); },
		py::is_operator())
	    .def("__repr__",
		 [suffix](const Q &q) {
			 std::ostringstream streamBuffer;
			 streamBuffer << std::scientific
...
void init_quantity(py::module &m) {
	...
	try {
		...
		std::cerr
		    << "No astropy.units package found. For advanced units "
		       "features, please install astropy."
		    << std::endl;
		apyu.enabled = false;
	}
	...
	declare_quantity<QEnergyOverTemperature>(m, "QEnergyOverTemperature",
						 "J K^-1", apyu);
	...
	declare_quantity<QPDensityPerEnergy>(m, "QPDensityPerEnergy",
					     "m^-3 J^-1", apyu);
	...
	// |- unnamed
	declare_quantity<QGREmissivity>(m, "QGREmissivity", "m^-5 kg^-1 s",
					apyu);
	declare_quantity<QICInnerIntegral>(m, "QICInnerIntegral",
					   "m^-3 kg^-1 s^2", apyu);
}
