...
TEST(Interactions, AAfrag) {
	...
	EXPECT_NEAR(static_cast<double>(sigma), static_cast<double>(0.0200034_mbarn / 1_GeV), 1e-4);
	sigma = aafragnu->getDiffCrossSection(Proton, Proton, E_proton, E_secondary);
	EXPECT_NEAR(static_cast<double>(sigma), static_cast<double>(0.0211308_mbarn / 1_GeV), 1e-4);

	E_proton = 1_TeV;
	E_secondary = 10_GeV;
	sigma = aafraggamma->getDiffCrossSection(Proton, Proton, E_proton, E_secondary);
	EXPECT_NEAR(static_cast<double>(sigma), static_cast<double>(7.03366_mbarn / 1_GeV), 1e-4);
	sigma = aafragnu->getDiffCrossSection(Proton, Proton, E_proton, E_secondary);
	EXPECT_NEAR(static_cast<double>(sigma), static_cast<double>(14.6908_mbarn / 1_GeV), 1e-4);
}
