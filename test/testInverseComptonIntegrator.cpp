...
TEST(InverseComptonIntegrator, integrateOverPhotonEnergyCMB) {
	...
	auto photonField =
	    std::make_shared<photonfields::CMB>(photonfields::CMB());
	...
	auto res = intIC->integrateOverPhotonEnergy(Vector3QLength(0), Egamma,
						    Eelectron);
...
TEST(InverseComptonIntegrator, compareLOSIntegrations) {
	...
	auto result_QAG = gslQAGIntegration<QDifferentialFlux, QGREmissivity>(
	    [maxDist, dir, integrand](QLength dist) { return integrand(dist); },
	    0, maxDist, 300);
	...
	EXPECT_NEAR(static_cast<double>(result_QAG),
		    static_cast<double>(result_SI), 1e-5);
}
