...
TEST(SynchroIntegrator, totalEnergyLoss) {
	...
	expectedEnergyLoss = 2 * sigma_Thomson * c_light * U_mag *
	                     std::pow(sin(90_deg), 2) *
	                     pow<2>(getLorentzFactor(m_electron, electronEnergy));

	// EXPECT_NEAR(static_cast<double>(expectedEnergyLoss),
	// static_cast<double>(totalEnergyLoss), 1e-32);
}
...
TEST(SynchroIntegrator, integrateOverEnergy) {
	 ...
	 * J =
	 * sqrt(3)*electron^3/(8*pi^2*epsilon_0*c*electron_mass)*dn/dE*F(1)*B_perp*deltaE
	 */
	...
	QEmissivity emissivity =
	    integrator->integrateOverEnergy(Vector3QLength(1_pc, 0, 0), 1_MHz);

	// sqrt(3)*e_charge^3/(8*pi^2*epsilon_0*c*electron_mass)*0.655*1*microGauss*1/(m^3*J)*1_eV
	// EXPECT_NEAR(static_cast<double>(emissivity), 3.915573e-55, 2e-56); //
	// J/m^3
}
...
TEST(SynchroIntegrator, integrateOverLOS) {
	...
	QTemperature T_expected =
	    intensityToTemperature(QIntensity(1.3892e-34) / 4_pi, freq);
