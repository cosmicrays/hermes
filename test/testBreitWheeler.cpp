...
TEST(Interactions, BreitWheeler) {
	auto bw = std::make_shared<interactions::BreitWheeler>(interactions::BreitWheeler());

	QEnergy E_ph = 1_eV;
	QEnergy E_gamma = 300_TeV;
	QArea res = bw->integratedOverTheta(E_gamma, E_ph);
	EXPECT_NEAR(static_cast<double>(res), static_cast<double>(2.80_mbarn), static_cast<double>(0.01_mbarn));

	E_ph = 1_eV;
	E_gamma = 50_TeV;
	res = bw->integratedOverTheta(E_gamma, E_ph);
	EXPECT_NEAR(static_cast<double>(res), static_cast<double>(12.30_mbarn), static_cast<double>(0.01_mbarn));
}
