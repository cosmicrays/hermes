...
TEST(YMW16, compareValues) {
	...
	auto gdensity =
	    std::make_shared<chargedgas::YMW16>(chargedgas::YMW16());
	...
	EXPECT_NEAR(
	    ...
	    static_cast<double>(gdensity->getDensity(Vector3QLength(0, 0, 0))),
	    1);
	EXPECT_NEAR(static_cast<double>(3.891344 / 1_cm3),
		    static_cast<double>(
			gdensity->getDensity(Vector3QLength(0, 0, 15_pc))),
		    1);
	...
	EXPECT_NEAR(static_cast<double>(0.17535603 / 1_cm3),
		    static_cast<double>(
			gdensity->getDensity(Vector3QLength(300_pc, 0, 0))),
		    1);
	...
	EXPECT_NEAR(static_cast<double>(0.001527 / 1_cm3),
		    static_cast<double>(gdensity->getDensity(
			Vector3QLength(0_pc, -10_kpc, 3_kpc))),
		    1);
}
