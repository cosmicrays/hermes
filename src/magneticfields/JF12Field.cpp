...
Vector3QMField JF12Field::getRegularField(const Vector3QLength &pos) const {
	...
	if (r > 3_kpc) {
		...
		if (r < 5_kpc) {
			...
			if (r_negx > rArms[7]) {
				r_negx = r * exp((-(phi + QAngle(3 * pi)) / tan90MinusPitch) /
				                 1_rad);
			}

			for (int i = 7; i >= 0; i--) {
				if (r_negx < rArms[i]) bMag = static_cast<double>(bDisk[i]);
			}

			bMag *= static_cast<double>((5_kpc / r) * (QNumber(1) - lfDisk));
			b.x += bMag *
			       static_cast<double>(sinPitch * cosPhi - cosPitch * sinPhi);
			b.y += bMag *
			       static_cast<double>(sinPitch * sinPhi + cosPitch * cosPhi);
		}
	}
...
QMField JF12Field::getTurbulentStrength(const Vector3QLength &pos) const {
	...
	if (r < 5_kpc) {
		...
		for (int i = 7; i >= 0; i--) {
			if (r_negx < rArms[i]) bDisk = bDiskTurb[i];
		}
	...
	// halo
	QMField bHalo =
	    bHaloTurb * exp(-r / rHaloTurb) * exp(-0.5 * pow<2>(pos.z / zHaloTurb));
...
Vector3QMField JF12Field::getField(const Vector3QLength &pos_) const {
	...
	if (useTurbulent) b += getTurbulentField(pos);
	if (useStriated) {
