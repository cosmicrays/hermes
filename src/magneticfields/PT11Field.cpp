...
void PT11Field::setUseASS(bool use) {
	...
	if (useBSS) {
		std::cout << "PT11Field: Disk field changed to ASS"
			  << std::endl;
		useBSS = false;
	}
...
void PT11Field::setUseBSS(bool use) {
	...
	if (useASS) {
		std::cout << "PT11Field: Disk field changed to BSS"
			  << std::endl;
		useASS = false;
	}
...
Vector3QMField PT11Field::getField(const Vector3QLength &pos_) const {
	...
	if ((useASS) or (useBSS)) {
		...
		QAngle theta =
		    QAngle(pi) -
		    pos.getPhi(); // azimuth angle theta: PT11 paper uses
		...
		// sin(pitch)
		b.x = static_cast<double>(sin_pitch * cos_theta -
					  cos_pitch * sin_theta);
		b.y = -static_cast<double>(sin_pitch * sin_theta -
					   cos_pitch * cos_theta);
		...
		double bMag = static_cast<double>(
		    cos(theta - 1_rad * cos_pitch / sin_pitch * log(r / R_sun) +
			PHI));
		if (useASS)
			bMag = fabs(bMag);
		bMag *= static_cast<double>(B0_D * R_sun / std::max(r, R_c) /
					    cos_PHI * exp(-fabs(pos.z) / z0_D));
		b *= bMag;
	}
	...
	if (useHalo) {
		...
		QLength z1 =
		    static_cast<double>(fabs(pos.z) < z0_H ? z11_H : z12_H);
