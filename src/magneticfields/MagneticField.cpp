...
Vector3QMField PeriodicMagneticField::getField(
    const Vector3QLength &position) const {
	Vector3d n =
...
Vector3QMField MagneticFieldList::getField(
    const Vector3QLength &position) const {
	Vector3QMField b;
	for (int i = 0; i < fields.size(); i++) b += fields[i]->getField(position);
	return b;
}
