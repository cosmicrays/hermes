...
void FITSFile::createImage(FITS::ImgType bitpix, int naxis, long *naxes) {
	if (fits_create_img(fptr, static_cast<int>(bitpix), naxis, naxes,
			    &status))
		fits_report_error(stderr, status);
	if (status != 0)
...
void FITSFile::writeImage(FITS::DataType dataType, int firstElement,
	...
	if (fits_write_img(fptr, static_cast<int>(dataType), firstElement,
...
void FITSFile::createTable(FITS::HDUType tableType, long int nRows,
			   int nColumns, char *columnName[], char *columnType[],
...
void FITSFile::writeColumn(FITS::DataType dataType, int column,
			   long int firstRow, long int firstElement,
