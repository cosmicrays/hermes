...
void Sun08Field::setUseHalo(bool use) {
	useHalo = use;
}
...
bool Sun08Field::isUsingHalo() const {
	return useHalo;
}
...
Vector3QMField Sun08Field::getRegularField(const Vector3QLength& pos_) const {
	...
	if (rho > disk_R_1) D2 = 1;
	if (rho > disk_R_2 && rho <= disk_R_1) D2 = -1;
	if (rho > disk_R_3 && rho <= disk_R_2) D2 = +1;
	if (rho <= disk_R_3) D2 = -1;
	...
	if (useHalo) {
		B_r = 0_muG;
		B_phi = halo_B_0 * 
			1 / (QNumber(1) + pow<2>((fabs(pos_.z) - halo_z_0)/halo_z_1)) *
