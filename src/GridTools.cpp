...
void initTurbulence(std::shared_ptr<VectorGrid> grid, double Brms, double lMin,
	...
	if ((Nx != Ny) or (Ny != Nz))
		throw std::runtime_error(
		    "turbulentField: only cubic grid supported");

	Vector3d spacing = grid->getSpacing();
	if ((spacing.x != spacing.y) or (spacing.y != spacing.z))
		throw std::runtime_error(
		    "turbulentField: only equal spacing suported");
		...
		for (size_t iy = 0; iy < n; iy++) {
			for (size_t iz = 0; iz < n2; iz++) {
				...
				if (helicity == true) {
					...
					double Bkprefactor =
					    static_cast<double>(mu0) /
					    (4 * M_PI * std::pow(k, 3));
					Bktot = fabs(random.randNorm() *
						     std::pow(k, alpha / 2));
					Bkplus = Bkprefactor *
						 sqrt((1 + H) / 2) * Bktot;
					Bkminus = Bkprefactor *
					...
					Bkx[i][0] =
					    ((Bkplus * ctp + Bkminus * ctm) *
						 e1.x +
					     (-Bkplus * stp + Bkminus * stm) *
						 e2.x) /
					    sqrt(2);
					Bkx[i][1] =
					    ((Bkplus * stp + Bkminus * stm) *
					...
					Vector3f BkRe(Bkx[i][0], Bky[i][0],
						      Bkz[i][0]);
					Vector3f BkIm(Bkx[i][1], Bky[i][1],
						      Bkz[i][1]);
	...
	float *Bx = (float *)Bkx;
	fftwf_plan plan_x =
	    fftwf_plan_dft_c2r_3d(n, n, n, Bkx, Bx, FFTW_ESTIMATE);
	fftwf_execute(plan_x);
	fftwf_destroy_plan(plan_x);

	float *By = (float *)Bky;
	fftwf_plan plan_y =
	    fftwf_plan_dft_c2r_3d(n, n, n, Bky, By, FFTW_ESTIMATE);
	fftwf_execute(plan_y);
	fftwf_destroy_plan(plan_y);

	float *Bz = (float *)Bkz;
	fftwf_plan plan_z =
	    fftwf_plan_dft_c2r_3d(n, n, n, Bkz, Bz, FFTW_ESTIMATE);
	...
	for (size_t ix = 0; ix < Nx; ix++)
		for (size_t iy = 0; iy < Ny; iy++)
			for (size_t iz = 0; iz < Nz; iz++) {
				Vector3d pos =
				    Vector3d(double(ix) + 0.5, double(iy) + 0.5,
					     double(iz) + 0.5) *
					spacing +
				    origin;
				Vector3d B = field->getField(
				    static_cast<Vector3QLength>(pos));
				grid->get(ix, iy, iz) = B;
			}
}
...
void loadGrid(std::shared_ptr<VectorGrid> grid, std::string filename,
	...
	if (length != (3 * nx * ny * nz))
		throw std::runtime_error(
		    "loadGrid: file and grid size do not match");
	...
	for (int ix = 0; ix < grid->getNx(); ix++) {
		for (int iy = 0; iy < grid->getNy(); iy++) {
			for (int iz = 0; iz < grid->getNz(); iz++) {
				...
				if (fin.eof())
					throw std::runtime_error(
					    "load VectorGrid: file too short");
			}
		}
	}
	fin.close();
	...
	for (int ix = 0; ix < grid->getNx(); ix++) {
		for (int iy = 0; iy < grid->getNy(); iy++) {
			for (int iz = 0; iz < grid->getNz(); iz++) {
				...
				if (fin.eof())
					throw std::runtime_error(
					    "load ScalarGrid: file too short");
			}
		}
	}
	fin.close();
