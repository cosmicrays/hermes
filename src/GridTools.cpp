...
#include "fftw3.h"
void initTurbulence(std::shared_ptr<VectorGrid> grid, double Brms, double lMin, double lMax, double alpha, int seed, bool helicity, double H) {
	size_t Nx = grid->getNx();
	size_t Ny = grid->getNy();
	size_t Nz = grid->getNz();
	if ((Nx != Ny) or (Ny != Nz))
	...
	for (size_t ix = 0; ix < n; ix++) {
		for (size_t iy = 0; iy < n; iy++) {
			for (size_t iz = 0; iz < n2; iz++) {
				...
				if (helicity == true) {
					if (ek.getAngleTo(n0) < 1e-3_rad) { // ek parallel to (1,1,1)
						e1.setXYZ(-1, 1, 0);
						e2.setXYZ(1, 1, -2);
					...
					double Bkprefactor = static_cast<double>(mu0) / (4 * M_PI * std::pow(k, 3));
...
double turbulentCorrelationLength(double lMin, double lMax, double alpha) {
	double r = lMin / lMax;
	double a = -alpha - 2;
	return lMax / 2 * (a - 1) / a * (1 - std::pow(r, a)) / (1 - std::pow(r, a - 1));
}

void loadGrid(std::shared_ptr<VectorGrid> grid, std::string filename, double c) {
...
void loadGrid(std::shared_ptr<ScalarGrid> grid, std::string filename, double c) {
	std::ifstream fin(filename.c_str(), std::ios::binary);
	if (!fin) {
		std::stringstream ss;
...
void dumpGrid(std::shared_ptr<VectorGrid> grid, std::string filename, double c) {
...
void dumpGrid(std::shared_ptr<ScalarGrid> grid, std::string filename, double c) {
	std::ofstream fout(filename.c_str(), std::ios::binary);
	if (!fout) {
		std::stringstream ss;
...
void loadGridFromTxt(std::shared_ptr<VectorGrid> grid, std::string filename, double c) {
...
void loadGridFromTxt(std::shared_ptr<ScalarGrid> grid, std::string filename, double c) {
	std::ifstream fin(filename.c_str());
	if (!fin) {
		std::stringstream ss;
		ss << "load ScalarGrid: " << filename << " not found";
...
void dumpGridToTxt(std::shared_ptr<VectorGrid> grid, std::string filename, double c) {
...
void dumpGridToTxt(std::shared_ptr<ScalarGrid> grid, std::string filename, double c) {
	std::ofstream fout(filename.c_str());
	if (!fout) {
		std::stringstream ss;
		ss << "dump ScalarGrid: " << filename << " not found";
