...
Vector3f meanFieldVector(const std::shared_ptr<VectorGrid>& grid) {
	...
	for (int ix = 0; ix < Nx; ix++)
		for (int iy = 0; iy < Ny; iy++)
			for (int iz = 0; iz < Nz; iz++) mean += grid->get(ix, iy, iz);
...
double rmsFieldStrength(const std::shared_ptr<ScalarGrid>& grid) {
	...
	for (int ix = 0; ix < Nx; ix++)
		for (int iy = 0; iy < Ny; iy++)
			for (int iz = 0; iz < Nz; iz++)
				sumV2 += std::pow(grid->get(ix, iy, iz), 2);
	return std::sqrt(sumV2 / Nx / Ny / Nz);
}
...
void initTurbulence(std::shared_ptr<VectorGrid> grid, double Brms, double lMin,
                    double lMax, double alpha, int seed, bool helicity,
                    double H) {
	size_t Nx = grid->getNx();
	size_t Ny = grid->getNy();
	size_t Nz = grid->getNz();
	...
	Random random;
	if (seed != 0) random.seed(seed);  // use given seed

	// calculate the n possible discrete wave numbers
	double K[n];
	for (int i = 0; i < n; i++) K[i] = (double)i / n - i / (n / 2);
	...
	for (size_t ix = 0; ix < n; ix++) {
		for (size_t iy = 0; iy < n; iy++) {
			for (size_t iz = 0; iz < n2; iz++) {
				...
				// (for helical fields together with the real
				// transform the following convention must be
				// used: e1(-k) = e1(k), e2(-k) = - e2(k)
				if (helicity == true) {
					if (ek.getAngleTo(n0) < 1e-3_rad) {  // ek parallel to
						                                 // (1,1,1)
						e1.setXYZ(-1, 1, 0);
						e2.setXYZ(1, 1, -2);
					...
					double Bkprefactor =
					    static_cast<double>(mu0) / (4 * M_PI * std::pow(k, 3));
					Bktot = fabs(random.randNorm() * std::pow(k, alpha / 2));
					...
					if (ek.isParallelTo(n0, 1e-3_rad)) {
					...
					// normal distributed amplitude with
					// mean = 0 and sigma = k^alpha/2
					b *= random.randNorm() * std::pow(k, alpha / 2);
	...
	for (size_t ix = 0; ix < Nx; ix++)
		for (size_t iy = 0; iy < Ny; iy++)
			for (size_t iz = 0; iz < Nz; iz++) {
				Vector3d pos = Vector3d(double(ix) + 0.5, double(iy) + 0.5,
				                        double(iz) + 0.5) *
				                   spacing +
				               origin;
		...
		for (size_t iy = 0; iy < Ny; iy++)
			for (size_t iz = 0; iz < Nz; iz++) {
				Vector3d pos = Vector3d(double(ix) + 0.5, double(iy) + 0.5,
				               ...
				               origin;
				QMField s =
				    field->getField(static_cast<Vector3QLength>(pos)).getR();
...
double turbulentCorrelationLength(double lMin, double lMax, double alpha) {
	double r = lMin / lMax;
	double a = -alpha - 2;
	return lMax / 2 * (a - 1) / a * (1 - std::pow(r, a)) /
	       (1 - std::pow(r, a - 1));
}

void loadGrid(const std::shared_ptr<VectorGrid>& grid,
              const std::string& filename, double c) {
...
void loadGrid(const std::shared_ptr<ScalarGrid>& grid,
              const std::string& filename, double c) {
	std::ifstream fin(filename.c_str(), std::ios::binary);
	if (!fin) {
		std::stringstream ss;
...
void dumpGrid(const std::shared_ptr<VectorGrid>& grid,
              const std::string& filename, double c) {
...
void dumpGrid(const std::shared_ptr<ScalarGrid>& grid,
              const std::string& filename, double c) {
	std::ofstream fout(filename.c_str(), std::ios::binary);
	if (!fout) {
		std::stringstream ss;
...
void loadGridFromTxt(const std::shared_ptr<VectorGrid>& grid,
                     const std::string& filename, double c) {
...
void loadGridFromTxt(const std::shared_ptr<ScalarGrid>& grid,
                     const std::string& filename, double c) {
	std::ifstream fin(filename.c_str());
	if (!fin) {
		std::stringstream ss;
...
void dumpGridToTxt(const std::shared_ptr<VectorGrid>& grid,
                   const std::string& filename, double c) {
...
void dumpGridToTxt(const std::shared_ptr<ScalarGrid>& grid,
                   const std::string& filename, double c) {
	std::ofstream fout(filename.c_str());
	if (!fout) {
		std::stringstream ss;
