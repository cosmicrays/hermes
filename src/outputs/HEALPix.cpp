...
HEALPix::HEALPix(const std::string &filename) : Output(), filename(filename) {}
...
void HEALPix::writeMetadata(int nside, double res,
...
void HEALPix::writeKeyValueAsDouble(const std::string &key, double value,
				    const std::string &description) {
    auto keyvalue = FITSKeyValue(key.c_str(), value);
    ffile->writeKeyValue(keyvalue, description.c_str());
}

void HEALPix::writeKeyValueAsString(const std::string &key,
