...
QDirection pix2ang_ring(unsigned int nside, unsigned int ipix) {
	...
	const unsigned int ncap =
	    2 * nside * (nside - 1);  // ! points in each polar cap, =0 for nside =1
	...
	if (ipix1 <= ncap) {
		...
		const unsigned int iring = static_cast<unsigned>(std::floor(
		                               std::sqrt(hip - std::sqrt(fihip)))) +
		                           1;  // ! counted from North pole
		const unsigned int iphi =
		    ipix1 - 2 * iring * (static_cast<int>(iring) - 1);
	...
	if (ipix1 <= nl2 * (5 * nside + 1)) {
		...
		const double fodd =
		    0.5 *
		    (1 + std::fmod(static_cast<double>(iring + nside),
		                   2));  //  ! 1 if iring+nside is odd, 1/2 otherwise

		thetaphi[0] = std::acos(
		    (static_cast<int>(nl2) - static_cast<int>(iring)) / fact1);
	...
	//! South Polar cap
	{
		...
		const unsigned iphi = static_cast<int>(
		    4. * iring + 1 -
		    (static_cast<int>(ip) -
		     2. * static_cast<int>(iring) * (static_cast<int>(iring) - 1)));
...
unsigned int loc2pix(unsigned int nside, double z, double phi, double sth,
	...
	// North & South polar caps
	{
		...
		const long ip =
		    std::floor(static_cast<double>(ir) * tt);  // in {0,4*ir-1}
