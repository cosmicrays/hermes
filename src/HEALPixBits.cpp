...
QDirection pix2ang_ring(unsigned int nside, unsigned int ipix) {
	...
	if( ipix1 <= ncap ) {
		...
		const unsigned int iring = (unsigned)std::floor( std::sqrt(hip - std::sqrt(fihip) ) ) + 1;// ! counted from North pole
	...
	if( ipix1 <= nl2*(5*nside+1) ) {
		...
		const unsigned int iring = (unsigned)std::floor( ip / nl4 ) + nside;// ! counted from North pole
...
unsigned int loc2pix(unsigned int nside, double z, double phi, double sth, bool have_sth) {
	...
	if (za<=twothird) // Equatorial region
	{
		...
		const long jp = std::floor(temp1-temp2); // index of  ascending edge line
		const long jm = std::floor(temp1+temp2); // index of descending edge line

		// ring number counted from z=2/3
		const long ir = nside + 1 + jp - jm; // in {1,2n+1}
		const long kshift = 1-(ir&1); // kshift=1 if ir even, 0 otherwise

		const long t1 = jp+jm-nside+kshift+1+nl4+nl4;
		const long ip = (order>0) ?
	...
	// North & South polar caps
	{
		...
		const double tmp = ((za<0.99)||(!have_sth)) ?
			nside*std::sqrt(3*(1-za)) :
			nside*sth/std::sqrt((1.+za)/3.);

		const long jp = std::floor(tp*tmp); // increasing edge line index
		const long jm = std::floor((1.0-tp)*tmp); // decreasing edge line index

		const long ir = jp+jm+1; // ring number counted from the closest pole
