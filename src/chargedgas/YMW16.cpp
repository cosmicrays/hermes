...
QPDensity YMW16::getDensity(const Vector3QLength &pos) const {
	auto conversion = [](QLength x) {
		return static_cast<double>(x / parsec);
	};

	double x =
	    -1 *
	    conversion(pos.getY()); // change coordinates X<>Y since in YMW16
	double y = conversion(
	    pos.getX()); // the sun position is at (0,+8.3_kpc,6.0_pc)
...
void YMW16::initParameters() {
	...
	double Ele_arm[] = {
	    // Ele arm
	    P_Ele_arm_0, P_Ele_arm_1, P_Ele_arm_2, P_Ele_arm_3, P_Ele_arm_4,
	};
	double Wid_arm[] = {
	    // Wid arm
	    P_Wid_arm_0, P_Wid_arm_1, P_Wid_arm_2, P_Wid_arm_3, P_Wid_arm_4,
	};
	...
	t7 = {
	    P_nLI, P_RLI, P_WLI, P_detthetaLI, P_thetaLI,
	};
...
double YMW16::galcen(double xx, double yy, double zz) const {
	...
	double Rgc =
	    std::sqrt((xx - Xgc) * (xx - Xgc) + (yy - Ygc) * (yy - Ygc));

	if (Rgc > (mc * t4.Agc) || std::fabs(zz) > (mc * t4.Hgc)) {
		return 0;
	} else {
		gc = 1;
		...
		for (i = 0; i <= 4; i++) {
			...
			if (i == 0) {
				if (theta >= 0 && theta < 0.77) {
					armr = rmin[i] *
					       exp((theta + 2 * pi - thmin[i]) *
						   tpitch[i]);
					detrr = std::fabs(rr - armr);
				}
				if (theta >= 0.77 && theta < 6.28) {
					armr1 = rmin[i] *
						std::exp((theta - thmin[i]) *
							 tpitch[i]);
					armr2 =
					    rmin[i] * std::exp((theta + 2 * pi -
								thmin[i]) *
							       tpitch[i]);
					detrr = MIN(std::fabs(rr - armr1),
						    std::fabs(rr - armr2));
				}
			}
			// Perseus
			...
			sech2 =
			    std::pow((2 / (std::exp((rr - t3.B2s) / t3.Aa) +
...
double YMW16::gum(double xx, double yy, double zz, int *m_5) const {
	...
	theta =
	    std::fabs(std::atan((zz - zc) / std::sqrt((xx - xc) * (xx - xc) +
						      (yy - yc) * (yy - yc))));
...
double YMW16::lmc(double l, double b, double d, int *w_lmc) const {
	...
	s_delta =
	    sin(b) * sin(delta_cp) + cos(b) * cos(delta_cp) * cos(l_cp - l);
	...
	c_dag =
	    c_delta * cos(delta_l) * (c_alpha_cp * c_cpl - s_alpha_cp * s_cpl) +
	    s_delta * sin(delta_l);
...
double YMW16::dora(double l, double b, double d) const {
	...
	sc_dp = -c_delta * (s_alpha_cp * c_cpl + c_alpha_cp * s_cpl);
	ss_dp = s_delta * std::cos(delta_l) -
		c_delta * std::sin(delta_l) *
		    (c_alpha_cp * c_cpl - s_alpha_cp * s_cpl);
	//(xc,yc,zc,iag,npag)------get(X,Y,Z) is (x',y',z')
	double X = d * (sc_dp * std::cos(npag) + ss_dp * std::sin(npag));
	double Y = d * (std::cos(iag) *
			    (ss_dp * std::cos(npag) - sc_dp * std::sin(npag)) +
			c_dag * std::sin(iag)) -
		   D_l * std::sin(iag);
	double Z = d * (std::sin(iag) *
			    (ss_dp * std::cos(npag) - sc_dp * std::sin(npag)) -
			c_dag * std::cos(iag)) +
		   D_l * std::cos(iag);
	...
	c_dagD = std::cos(delta_D) * std::cos(delta_l) *
		     std::cos(alpha_D - alpha_l) +
...
double YMW16::ne_ymw16(const Vector3QLength &pos) const {
	...
	if (rr < R_warp) {
		...
		z_warp =
		    t0.Gamma_w * (rr - R_warp) * cos(theta_warp - theta_max);
		zz_w = zz - z_warp;
	}
	...
	if (R_g < 100000) {
		...
		if (np == 1) {
			...
			ne = 0; // ("The point is located outside of MW and
				// MC.\n");
		}
	} else {
