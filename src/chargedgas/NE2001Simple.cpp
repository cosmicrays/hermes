...
QPDensity
NE2001Simple::getGalacticCentreDensity(const Vector3QLength &pos) const {
	QLength r = pos.getR();
	QLength dr = sqrt((pos.x - x_GC) * (pos.x - x_GC) +
			  (pos.y - y_GC) * (pos.y - y_GC));
	...
	// paper
	QNumber g_GC = (dr * dr / (r_GC * r_GC) +
			dz * dz / (H_GC * H_GC)); // original code
