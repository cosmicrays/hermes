...
WMAP07CRDensity::WMAP07CRDensity(QEnergy minE_, QEnergy maxE_, int steps_) : minE(minE_), maxE(maxE_), steps(steps_) {
	makeEnergyRange();
	setParameters();
}
...
QPDensityPerEnergy WMAP07CRDensity::getDensityPerEnergy(const QEnergy& E_, const Vector3QLength& pos_) const {
