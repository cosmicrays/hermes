...
QPDensityPerEnergy
Dragon2D::getDensityPerEnergy(const QEnergy &E_,
			      const Vector3QLength &pos_) const {
	return getDensityPerEnergy(static_cast<int>(energyIndex.at(E_)), pos_);
}

QPDensityPerEnergy
Dragon2D::getDensityPerEnergy(int iE_, const Vector3QLength &pos_) const {
...
void Dragon2D::readSpatialGrid2D() {
	...
	// zmin.getValue());
	Vector3d origin(-1 * static_cast<double>(rmax),
			static_cast<double>(zmin), 0);
	Vector3d spacing(static_cast<double>(deltar),
			 static_cast<double>(deltaz), 0);

	for (int i = 0; i < dimE; ++i) {
		grid.push_back(std::make_unique<ScalarGrid2DQPDensityPerEnergy>(
		    ScalarGrid2DQPDensityPerEnergy(origin, dimr, dimz,
						   spacing)));
	}
}
