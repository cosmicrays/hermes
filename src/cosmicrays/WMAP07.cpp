...
WMAP07::WMAP07() : minE(1_GeV), maxE(1e4_GeV), steps(10) {
	makeEnergyRange();
	setParameters();
}

WMAP07::WMAP07(QEnergy minE_, QEnergy maxE_, int steps_)
    : minE(minE_), maxE(maxE_), steps(steps_) {
	makeEnergyRange();
	setParameters();
}

void WMAP07::setParameters() {
	spectralIndex = 3;
	h_d = 1_kpc;
	h_r = 5_kpc;
	E_0 = 1_GeV;
...
QPDensityPerEnergy WMAP07::getDensityPerEnergy(
