...
Dragon3D::Dragon3D(const std::string &filename_, const std::vector<PID> &pids_)
...
QPDensityPerEnergy
Dragon3D::getDensityPerEnergy(const QEnergy &E_,
			      const Vector3QLength &pos_) const {
	return getDensityPerEnergy(static_cast<int>(energyIndex.at(E_)), pos_);
}

QPDensityPerEnergy
Dragon3D::getDensityPerEnergy(int iE_, const Vector3QLength &pos_) const {
...
void Dragon3D::readEnergyAxis() {
	...
	for (int i = 0; i < dimE; ++i) {
		E = 1_GeV *
		    std::exp(std::log(Ekmin) + static_cast<double>(i) *
...
void Dragon3D::readDensity3D() {
	...
	while (hduActual < hduNumber) {
		ffile->moveToHDU(
		    hduIndex); // Move to the next HDU (the first HDU = 1)
		...
		if (isPIDEnabled(PID(Z, A))) {
			...
			std::cerr << "... reading species with Z = " << Z
				  << " A = " << A << " at HDU = " << hduActual
				  << std::endl;

			std::vector<float> rawData =
			    ffile->readImageAsFloat(firstElement, nElements);

			std::size_t iE, ix, iy, iz;
			std::div_t dv;
			for (auto it = rawData.begin(); it != rawData.end();
				...
				grid[iE]->addValue(
				    ix, iy, iz,
				    static_cast<QPDensityPerEnergy>(*it));
			}
		}
		hduIndex++;
	}
