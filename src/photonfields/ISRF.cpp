...
ISRF::ISRF() {
	...
	setStartEnergy(logWavelenghtToFrequency(logwavelenghts.back()) *
		       h_planck);
	setEndEnergy(logWavelenghtToFrequency(logwavelenghts.front()) *
		     h_planck);
...
QEnergyDensity ISRF::getEnergyDensity(const QLength &r, const QLength &z,
	...
	std::size_t ifreq = std::lower_bound(logwavelenghts.begin(),
					     logwavelenghts.end(), logf_) -
	...
	*/
	...
	double c_00 = getISRF(ir, iz, ifreq) * (1. - r_d) +
		      getISRF(ir + 1, iz, ifreq) * r_d;
