...
FreeFreeIntegrator::FreeFreeIntegrator(const std::shared_ptr<ionizedgas::IonizedGasDensity> &gdensity_)
...
QNumber FreeFreeIntegrator::gauntFactor(const QFrequency &freq, const QTemperature &T, int Z) const {
	...
	const auto K = (128 * pow<2>(epsilon0) * pow<3>(k_boltzmann)) / (m_electron * pow<4>(e_plus));
...
QEmissivity FreeFreeIntegrator::spectralEmissivity(const Vector3QLength &pos_, const QFrequency &freq_) const {
	int Z = 1;
	QPDensity N, N_e;
...
QInverseLength FreeFreeIntegrator::absorptionCoefficient(const Vector3QLength &pos_, const QFrequency &freq_) const {
	QTemperature T = 1e4_K;

	return spectralEmissivity(pos_, freq_) * c_squared / (8_pi * h_planck * pow<3>(freq_)) *
	       expm1(h_planck * freq_ / (k_boltzmann * T));
}

QEmissivity FreeFreeIntegrator::spectralEmissivityExplicit(const QPDensity &N, const QPDensity &N_e,
                                                           const QFrequency &freq, const QTemperature &T, int Z) const {
	// optimisation of constants
	const auto K = 1.0 / (3 * pi * pi) * std::sqrt(pi / 6) * pow<6>(e_plus) /
	               (pow<3>(epsilon0 * c_light) * pow<2>(m_electron)) * sqrt(m_electron / (k_boltzmann));

	return K * Z * Z * gauntFactor(freq, T, Z) * N * N_e * sqrt(1 / T) * exp(-h_planck * freq / (k_boltzmann * T));
}
