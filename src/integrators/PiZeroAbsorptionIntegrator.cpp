...
void PiZeroAbsorptionIntegrator::initCacheTable() {
	...
	for (auto &c : job_chunks) {
		threads.push_back(std::thread(
		    &PiZeroAbsorptionIntegrator::computeCacheInThread, this,
...
QPiZeroIntegral
PiZeroAbsorptionIntegrator::getIOEfromCache(Vector3QLength pos_,
					    QEnergy Egamma_) const {
	return cacheTable->interpolate(static_cast<Vector3d>(pos_));
}

QDifferentialIntensity
PiZeroAbsorptionIntegrator::integrateOverLOS(QDirection direction) const {
	return integrateOverLOS(direction, 1_GeV);
}

QDifferentialIntensity
PiZeroAbsorptionIntegrator::integrateOverLOS(QDirection direction_,
					     QEnergy Egamma_) const {
	...
	for (const auto &ring : *ngdensity) {
		...
		auto normI_f = [ring, this](const Vector3QLength &pos) {
			return (ring->isInside(pos)) ? this->densityProfile(pos)
						     : 0;
		};
		auto normIntegrand = [this, normI_f,
				      direction_](const QLength &dist) {
			return normI_f(getGalacticPosition(this->positionSun,
							   dist, direction_));
		};
		...
		auto losIntegrand = [this, losI_f, direction_,
			...
			return losI_f(getGalacticPosition(this->positionSun,
							  dist, direction_),
				      Egamma_);
		};
...
QPDensity
PiZeroAbsorptionIntegrator::densityProfile(const Vector3QLength &pos) const {
	return QPDensity(1);
}

QRingX0Unit
PiZeroAbsorptionIntegrator::X0Function(const Vector3QLength &pos) const {
	return 1.8e20 / (1_cm2 * 1_K * 1_km) * 1_s;
}

QPiZeroIntegral
PiZeroAbsorptionIntegrator::integrateOverEnergy(Vector3QLength pos_,
						QEnergy Egamma_) const {
	if (cacheTableInitialized)
	...
	for (const auto &crDensity : crList) {
		...
		for (const auto &neutralGas :
		     ngdensity->getAbundanceFractions()) {
			auto pid_target = neutralGas.first;
			auto f_target = neutralGas.second;
			total +=
			    f_target *
			    crossSec->getSigma(pid_projectile, pid_target) *
			    integralOverEnergy;
		}
	}
	return total;
