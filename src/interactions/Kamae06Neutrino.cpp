...
Kamae06Neutrino::Kamae06Neutrino() :
...
void Kamae06Neutrino::enableCaching() {
	cachingEnabled = true;
};

void Kamae06Neutrino::disableCaching() {
	cachingEnabled = false;
};

QDifferentialCrossSection Kamae06Neutrino::getDiffCrossSection(
...
QDifferentialCrossSection Kamae06Neutrino::getDiffCrossSectionDirectly(
		const QEnergy &E_proton,
		const QEnergy &E_nu) const {
	...
	return QDifferentialCrossSection(
		dsigma_dlogTp / E_nu_GeV * (1_mbarn / 1_GeV));
}
