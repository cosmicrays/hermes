...
QNumber BremsstrahlungSimple::ElwertFactor(const QNumber &beta_i,
	...
	return beta_i * (1. - exp(-2.0 * pi * Z * alpha_fine / beta_i)) /
	       beta_f * (1. - exp(-2.0 * pi * Z * alpha_fine / beta_f));
}
...
inline double BremsstrahlungSimple::R_2(double q, double Z) const {
	double F_2 =
	    1. / pow<2>(1. + pow<2>(q) /
				 pow<2>(2. * static_cast<double>(alpha_fine) *
					(Z - 5. / 16.)));
...
QNumber BremsstrahlungSimple::I_Phi_2(const QNumber &delta_, int Z,
	...
	auto R_N = [this, delta, Z, N](double q) {
		return ((N == 1) ? R_1(q, Z) : R_2(q, Z)) / pow<4>(q) *
		       (pow<3>(q) -
			6. * pow<2>(delta) * q * std::log(q / delta) +
...
QArea BremsstrahlungSimple::dsdk_IntermediateEnergy(
	...
	factor -= 2. * gamma_i * gamma_f * (pow<2>(p_f) + pow<2>(p_i)) /
		  pow<2>(p_f * p_i);
	factor += epsilon_i * gamma_f / pow<3>(p_i) +
		  epsilon_f * gamma_i / pow<3>(p_f);
