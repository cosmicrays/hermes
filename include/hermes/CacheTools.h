...
struct pair_hash : public std::unary_function<key_t, std::size_t> {
...
struct pair_equal : public std::binary_function<tPairKey, tPairKey, bool>
{
   ...
   {
      return (
               areEqual(v0.first, v1.first) &&
               areEqual(v0.second, v1.second)
             );
   }
};
...
struct array_equal : public std::binary_function<tArrayKey, tArrayKey, bool>
{
   ...
   {
      return (
               areEqual(std::get<0>(v0), std::get<0>(v1)) &&
...
typedef CacheStorageWith2Args<QEnergy,QEnergy,QDiffCrossSection> CacheStorageKamae;
typedef CacheStorageWith3Args<QEnergy,QEnergy,QEnergy,QDiffCrossSection> CacheStorageKN;
