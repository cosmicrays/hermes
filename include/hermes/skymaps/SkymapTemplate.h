...
template <typename QPXL, typename QSTEP> class SkymapTemplate : public Skymap {
      protected:
	typedef QPXL tPixel;
	...
	*/
	...
	SkymapTemplate(std::size_t nside,
...
SkymapTemplate<QPXL, QSTEP>::SkymapTemplate(std::size_t nside_, const QSTEP &p)
    : Skymap(nside_), skymapParameter(p), defaultOutputUnits(QPXL(1)),
...
void SkymapTemplate<QPXL, QSTEP>::compute() {
	std::cout << "hermes::Integrator: Number of Threads: "
	...
	for (auto &chunk : job_chunks) {
		threads.push_back(
		    std::thread(&SkymapTemplate<QPXL, QSTEP>::computePixelRange,
				this, chunk.first, chunk.second, integrator));
	}
	for (auto &t : threads) {
		t.join();
	}
...
template <typename QPXL, typename QSTEP>
QNumber
SkymapTemplate<QPXL, QSTEP>::toSkymapDefaultUnits(const QPXL pixel) const {
...
void SkymapTemplate<QPXL, QSTEP>::save(
	...
	std::vector<float>
	    tempArray; // allocate on heap, because of nside >= 512
	for (auto i : fluxContainer)
		tempArray.push_back(
