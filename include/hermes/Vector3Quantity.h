  ...
  public:
    ...
    */
	...
	if ((fabs(this->x) < eps) and (fabs(this->y) < eps) and
	    ...
	    return atan2((T)sqrt(this->x * this->x + this->y * this->y),
			 this->z);
    }
	...
	return Vector3Quantity<T>(std::floor(this->x), std::floor(this->y),
				  std::floor(this->z));
    }

    // return vector with ceiled values
    Vector3Quantity<T> ceil() const {
	return Vector3Quantity<T>(std::ceil(this->x), std::ceil(this->y),
				  std::ceil(this->z));
    }
    ...
    Vector3Quantity<T> operator%(const Vector3Quantity<T> &v) const {
	return Vector3Quantity<T>(fmod(this->x, v.x), fmod(this->y, v.y),
				  fmod(this->z, v.z));
    }
