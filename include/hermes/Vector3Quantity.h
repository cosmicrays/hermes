 ...
 */
template <typename T> class Vector3Quantity : public Vector3<T> {
      public:
	...
	Vector3Quantity(const Vector3Quantity<U> &v)
	    : Vector3<T>(static_cast<T>(v.getX()), static_cast<T>(v.getY()),
	...
	explicit Vector3Quantity(const T &X) : Vector3<T>(X, X, X) {}
	...
	*/
	...
	QAngle getTheta() const {
		T eps = std::numeric_limits<T>::min();
		if ((fabs(this->x) < eps) and (fabs(this->y) < eps) and
		    (fabs(this->z) < eps))
			return 0.0;
		else
			return atan2(
			    (T)sqrt(this->x * this->x + this->y * this->y),
			    this->z);
	}
	...
	QAngle getAngleTo(const Vector3Quantity<T> &v) const {
		double cosdistance =
		    static_cast<double>(this->dot(v) / v.getR() / this->getR());
	...
	Vector3Quantity<T> getRotated(const Vector3Quantity<T> &axis,
				      T angle) const {
		Vector3Quantity<T> u = axis;
		T c = cos(angle);
		T s = sin(angle);
		Vector3Quantity<T> Rx(c + u.x * u.x * (1 - c),
				      u.x * u.y * (1 - c) - u.z * s,
				      u.x * u.z * (1 - c) + u.y * s);
		Vector3Quantity<T> Ry(u.y * u.x * (1 - c) + u.z * s,
				      c + u.y * u.y * (1 - c),
				      u.y * u.z * (1 - c) - u.x * s);
	...
	Vector3Quantity<T> abs() const {
		return Vector3Quantity<T>(std::abs(this->x), std::abs(this->y),
					  std::abs(this->z));
	}

	// return vector with floored values
	Vector3Quantity<T> floor() const {
		return Vector3Quantity<T>(std::floor(this->x),
					  std::floor(this->y),
					  std::floor(this->z));
	}
	...
	Vector3Quantity<T> ceil() const {
		return Vector3Quantity<T>(
		    std::ceil(this->x), std::ceil(this->y), std::ceil(this->z));
	}
	...
	Vector3Quantity<T> cross(const Vector3Quantity<T> &v) const {
		return Vector3Quantity<T>(this->y * v.z - v.y * this->z,
	...
	Vector3Quantity<T> operator+(const Vector3Quantity<T> &v) const {
		return Vector3Quantity(this->x + v.x, this->y + v.y,
				       this->z + v.z);
	}
	...
	Vector3Quantity<T> operator/(const Vector3Quantity<T> &v) const {
		return Vector3Quantity<T>(this->x / v.x, this->y / v.y,
					  this->z / v.z);
	}

	Vector3Quantity<T> operator/(const T &f) const {
		return Vector3Quantity<T>(this->x / f, this->y / f,
					  this->z / f);
	}
	...
	Vector3Quantity<T> operator%(const Vector3Quantity<T> &v) const {
		return Vector3Quantity<T>(
		    fmod(this->x, v.x), fmod(this->y, v.y), fmod(this->z, v.z));
	}

	Vector3Quantity<T> operator%(const T &f) const {
		return Vector3Quantity<T>(fmod(this->x, f), fmod(this->y, f),
					  fmod(this->z, f));
	}
...
inline Vector3Quantity<Q> operator*(const Vector3Quantity<Q> &q,
				    const Vector3<T> &v) {
	return Vector3Quantity<Q>(v.x * q.x, v.y * q.y, v.z * q.z);
}
template <typename T, typename Q>
inline Vector3Quantity<Q> operator*(const Vector3<T> &v,
				    const Vector3Quantity<Q> &q) {
	return Vector3Quantity<Q>(v.x * q.x, v.y * q.y, v.z * q.z);
}
