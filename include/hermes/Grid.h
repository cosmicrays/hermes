...
/**
 @class Grid2D
 @brief Template class for fields on a periodic grid with trilinear
 interpolation

 The grid spacing is constant and equal along all three axes.
 Values are calculated by trilinear interpolation of the surrounding 8 grid
 points. The grid is periodically (default) or reflectively extended. The grid
  ...
  public:
	...
	*/
	...
	T closestValue(const Vector3d &position) const {
		...
		int iy = round(r.y);
		if (reflective) {
			while ((ix < 0) or (ix > Nx)) ix = 2 * Nx * (ix > Nx) - ix;
			while ((iy < 0) or (iy > Ny)) iy = 2 * Ny * (iy > Ny) - iy;
		} else {
			ix = ((ix % Nx) + Nx) % Nx;
			iy = ((iy % Ny) + Ny) % Ny;
		}
	...
	*/
	...
	T closestValue(const Vector3d &position) const {
		...
		if (reflective) {
			while ((ix < 0) or (ix > Nx)) ix = 2 * Nx * (ix > Nx) - ix;
			while ((iy < 0) or (iy > Ny)) iy = 2 * Ny * (iy > Ny) - iy;
			while ((iz < 0) or (iz > Nz)) iz = 2 * Nz * (iz > Nz) - iz;
		} else {
			ix = ((ix % Nx) + Nx) % Nx;
			iy = ((iy % Ny) + Ny) % Ny;
			iz = ((iz % Nz) + Nz) % Nz;
...
typedef Grid2D<QPDensityPerEnergy> ScalarGrid2DQPDensityPerEnergy;
